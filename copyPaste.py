...
def main():
	...
	originalSize = len(os.listdir(datasetPath+"/images/"+trainVal))
	...
	for image in os.listdir(datasetPath + "/images/train/"): 
		img = Image.open(datasetPath + "/images/train/" + image)
	...
	polyLabels = datasetPath + "/labels/" + trainVal + "Poly/"
	...
	polyLabels = datasetPath + "/labels/" + remaining + "Poly/"
	# Get the annotations from the dataset and save them into a dictionary
	# which key is the region in which the object belongs
	for labels in os.listdir(polyLabels):
		label = open(polyLabels + labels, "r")
	...
	for image in images:
		...
		y = list(range(0, height-resolution))
		bgCount = 0 
		while bgCount < bgLoops and len(x)>0 and len(y)>0:
			i = random.choice(x)
			j = random.choice(y)
			x.remove(i)
			y.remove(j)
